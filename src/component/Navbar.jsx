...
const Navbar = ({ user = null }) => {
  ...
  const ProfileButton = ({ icon, label, onClick, danger = false }) => {
    return (
      <button
        type="button"
        onClick={onClick}
        className={`flex w-full cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition-all duration-200 ${
          danger
      ...
      <AnimatePresence>
        {profileOpen && (
          <motion.div
            ...
            className="absolute right-0 top-[calc(100%+10px)] z-[100] w-[250px] max-w-[calc(100vw-24px)] overflow-hidden rounded-2xl border border-gray-100 bg-white p-1.5 shadow-[0_15px_45px_rgba(0,0,0,0.12)]"
          >
            {user ? (
              <>
                {/* USER INFO */}
                ...
                <ProfileButton
                  icon={<LuCircleUserRound size={18} />}
                  label="User Profile"
  ...
  const IconButton = ({ icon, label, onClick, className = "" }) => {
    return (
      <motion.button
        ...
        onClick={onClick}
        aria-label={label}
        className={`flex h-9 w-9 cursor-pointer items-center justify-center rounded-full text-gray-600 transition-colors duration-200 hover:bg-gray-50 hover:text-blue-500 ${className}`}
      >
        {icon}
      </motion.button>
    );
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gray-200">
            {userImage ? (
              ...
              <LuCircleUserRound size={24} className="text-gray-500" />
            )}
          </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            {role && (
              <p className="mt-1 text-[9px] font-bold uppercase tracking-wider text-blue-500">
                {role}
              </p>
            )}
          </div>
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed right-0 top-0 z-[9999] flex h-dvh w-[300px] max-w-[88vw] flex-col overflow-hidden bg-white shadow-2xl sm:hidden"
            >
              {/* HEADER */}
              ...
              <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-5 py-4">
                ...
                <motion.button
                  ...
                  className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-red-50 text-red-500 transition-colors hover:bg-red-100"
                >
                  <LuX size={20} />
                </motion.button>
              </div>
  ...
  return (
    <>
          ...
          <div className="flex shrink-0 items-center">
            ...
            {!user && (
              <div ref={profileRef} className="relative">
                <IconButton
                  icon={<LuCircleUserRound size={23} />}
                  label="Login"
            ...
            {user && isNormalUser && (
              <div className="flex items-center gap-5 sm:gap-8 md:gap-8">
                ...
                <div ref={profileRef} className="relative">
                  <IconButton
                    icon={<LuCircleUserRound size={24} />}
                    label="User Profile"
                    onClick={() => setProfileOpen((prev) => !prev)}
                  />
                ...
                <div className="sm:hidden">
                  <IconButton
                    icon={
                      mobileOpen ? (
                        <LuX size={23} className="text-red-500" />
                      ) : (
                        <HiMiniBars3 size={24} />
                      )
                    }
      ...
      {/* MOBILE SIDEBAR ONLY FOR LOGGED-IN USERS */}
