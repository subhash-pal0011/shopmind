...
const page = () => {
  ...
  const getAddressFromCoordinates = async (latitude, longitude ) => {
    try {
      ...
      toast.success(
        "Delivery address updated from map."
      );
    } catch (error) {
      console.error(
      ...
      toast.error(
        "Could not get address. Please enter it manually."
      );
    } finally {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        toast.info(
          "Location permission denied. You can select location manually."
        );
      },
  ...
  const isCODAvailable = useMemo(() => {
    ...
    return products.every(
      (item) =>
        item?.product?.payOnDelivery === true
    );
  }, [products]);
  ...
  const deliveryCharge = useMemo(() => {
    ...
    const hasPaidDelivery =
      products.some(
        (item) =>
          item?.product?.freeDelivery === false
      );
